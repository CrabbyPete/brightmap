...
class EventbriteClient(object):
    ...
    def __init__(self, app_key=None, user_key=None, user_id=None):
    ...
    def _execute_api_call(self, api_method, api_arguments, authenticate=True):
        ...
        if authenticate:
            assert self._app_key
            assert self._user_id or self._user_key
            url_arguments['app_key'] = self._app_key
            if self._user_id != None:
                url_arguments['user_id'] = self._user_id
            if self._user_key != None:
